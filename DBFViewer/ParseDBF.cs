...
// Reference to https://www.codeproject.com/Articles/24247/Load-a-DBF-into-a-DataTable
...
namespace DBFViewer
{
	public class ParseDBF
	{
		...
		// Read an entire standard DBF file into a DataTable
		public static DataTable ReadDBF(string dbfFile, Encoding encoding)
		{
					...
					foreach (FieldDescriptor field in fields)
					{
						switch (field.fieldType)
						{
							...
							case 'D': // Date (YYYYMMDD)
								year = encoding.GetString(recReader.ReadBytes(4));
								month = encoding.GetString(recReader.ReadBytes(2));
								day = encoding.GetString(recReader.ReadBytes(2));
